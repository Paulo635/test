...
class CookieManager:
    ...
    def save_cookies(self):
        ...
        try:
            ...
            print(f"💾 Cookies salvos em {self.cookie_file}")
        except Exception as e:
    ...
    def add_cookie(self, cookie: str):
        ...
        if cookie and cookie not in self.cookies:
            ...
            self.save_cookies()
            print(f"✅ Cookie adicionado. Total: {len(self.cookies)}")
    ...
    def remove_cookie(self, index: int):
        ...
        if 0 <= index < len(self.cookies):
            ...
            self.save_cookies()
            print(f"🗑️ Cookie removido: {removed[:50]}...")
...
class WplacePixelPainter:
    ...
    def paint_pixel(self, coord: WplaceCoordinate, color_index: int) -> bool:
        ...
        if not coord.is_valid():
            print(f"❌ [P{self.painter_id}] Coordenada inválida: Tl({coord.tl_x},{coord.tl_y}) Px({coord.px_x},{coord.px_y})")
            return False
        ...
        for attempt in range(max_retries):
            try:
                ...
                if response.status_code == 200:
                    ...
                    print(f"✅ [P{self.painter_id}] Pintado Tl({coord.tl_x},{coord.tl_y}) Px({coord.px_x},{coord.px_y}) -> Abs({abs_x},{abs_y}) Cor:{color_index}")
                    return True
                    ...
                    try:
                        ...
                        print(f"🚫 [P{self.painter_id}] Sem tinta! Charges: {charges:.3f}")
                        print(f"⏳ [P{self.painter_id}] Pausando 30 segundos para recarregar tinta...")
                        ...
                        print(f"❌ [P{self.painter_id}] Erro 403 sem dados JSON: {response.text}")
                        return False
                        
                else:
                    print(f"❌ [P{self.painter_id}] Erro HTTP {response.status_code}: {response.text}")
                    return False
                    
            except requests.RequestException as e:
                print(f"❌ [P{self.painter_id}] Erro de rede: {str(e)}")
                if attempt < max_retries - 1:
                    print(f"🔄 [P{self.painter_id}] Tentativa {attempt + 2}/{max_retries} em 5 segundos...")
                    time.sleep(5)
                else:
                    return False
        
        print(f"❌ [P{self.painter_id}] Falha após {max_retries} tentativas")
        return False
...
class MultiPainterCoordinator:
    ...
    def __init__(self, cookies: List[str], delay: float = 1.0):
        ...
        print(f"🎨 Coordenador inicializado com {len(self.painters)} painters")
...
class WplaceImageProcessor:
    ...
    def generate_paint_sequence(self, img: Image.Image, 
        ...
        print(f"🎯 Iniciando pela coordenada: Tl({start_coord.tl_x},{start_coord.tl_y}) Px({start_coord.px_x},{start_coord.px_y})")
        ...
        print(f"🌀 Sequência em espiral gerada: {len(paint_sequence)} pixels para pintar (centro → bordas)")
        print(f"📍 Centro da imagem: ({center_x}, {center_y})")
    ...
    def paint_image(self, image_path: str, start_coord: WplaceCoordinate, 
        ...
        if not paint_sequence:
            print("❌ Nenhum pixel válido para pintar")
            return False
    ...
    def resume_painting(self, output_json: Optional[str] = None) -> bool:
        ...
        if not progress:
            print("❌ Não foi possível carregar progresso")
            return False
        
        print("🔄 Continuando pintura de onde parou...")
        print(f"📊 Progresso anterior: {progress['completion_percentage']:.1f}%")
        ...
        print(f"🎯 Restam {len(remaining_sequence)} pixels para pintar")
    ...
    def _execute_painting(self, paint_sequence: List[Tuple[WplaceCoordinate, int]], 
        ...
        print(f"🚀 Usando {num_painters} painters para {len(paint_sequence)} pixels...")
